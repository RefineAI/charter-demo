...
def show_results(img, results, img_width, img_height, test):
    caffe_root = '/home/icarus/installs/caffe/'  # this file is expected to be in {caffe_root}/examples
    sys.path.insert(0, caffe_root + 'python')
    GPU_ID = 0  # Switch between 0 and 1 depending on the GPU you want to use.
    caffe.set_mode_gpu()
    ...
    for i in range(len(results)):
        ...
        if imshow:
            ...
            cv2.rectangle(test, (xmin, ymin - 20), (xmax, ymin), (125, 125, 125), -1)
            cv2.putText(test, results[i][0] + ' : %.2f' % results[i][5], (xmin + 5, ymin - 7),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    #cv2.imwrite("OrigImage.jpg", img)
    #cv2.imwrite("BeforeReturn.jpg", img_cp)
    return test
    #if imshow:
    #    cv2.imshow('Detection', img_cp)
    #    cv2.waitKey(1000)


def get_bounded_image(net, img_filename):
    caffe_root = '/home/icarus/installs/caffe/'  # this file is expected to be in {caffe_root}/examples
    sys.path.insert(0, caffe_root + 'python')
    GPU_ID = 0  # Switch between 0 and 1 depending on the GPU you want to use.
    caffe.set_mode_gpu()
    ...
    #Temp Fix
    test = cv2.imread(img_filename)
    bounded_img = show_results(img_cv, results, img.shape[1], img.shape[0], test)
    #cv2.imwrite("result.jpg", bounded_img)
    return bounded_img
